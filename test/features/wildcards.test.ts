...
test.serial('positionOf matches string wildcard with numbers and wildcards', (t) => {
	...
	// String wildcard matches numbers
	t.is(p.positionOf(['*', 'bar']), 1);
	t.is(p.positionOf(['foo', '*', 'bar']), 0);

	// String wildcard matches other wildcards
	t.is(p.positionOf(['foo', -1, 'bar']), 0);

	// String wildcard does NOT match regular strings
	t.is(p.positionOf(['*', 'nonexistent']), -1);
});
...
test.serial('positionOf with wildcards checks non-wildcard segments', (t) => {
	...
	t.is(p.positionOf(['foo', -1, 'wrong']), -1);
	t.is(p.positionOf(['wrong', -1, 'bar']), -1);
});
...
test.serial('positionOf with wildcards works with indices: Ignore option', (t) => {
	...
	t.is(p.positionOf(['*', 'bar'], { indices: Pathist.Indices.Ignore }), 1);

	// Wildcards match numbers
	t.is(p.positionOf(['foo', '*'], { indices: Pathist.Indices.Ignore }), 0);
	t.is(p.positionOf(['foo', -1], { indices: Pathist.Indices.Ignore }), 0);
});
...
test.serial('lastPositionOf matches string wildcard with numbers and wildcards', (t) => {
	...
	// String wildcard matches numbers
	t.is(p.lastPositionOf(['*', 'bar']), 5);
	t.is(p.lastPositionOf([1, 'baz']), 3);
	t.is(p.lastPositionOf(['*', 'baz']), 3);
...
test.serial('lastPositionOf with wildcards checks non-wildcard segments', (t) => {
	...
	// Wrong non-wildcard segments
	t.is(p.lastPositionOf(['foo', -1, 'wrong']), -1);
	t.is(p.lastPositionOf(['wrong', -1, 'bar']), -1);
});
...
test.serial('lastPositionOf with wildcards on both sides matches', (t) => {
	...
	t.is(p.lastPositionOf([99, 'bar']), 1);
});
...
test.serial('lastPositionOf finds last match with wildcards', (t) => {
	...
	// Compare with positionOf which finds first
	t.is(p.positionOf([-1, 'bar']), 1);
});
...
test.serial('lastPositionOf with wildcards works with indices: Ignore option', (t) => {
	...
	t.is(p.lastPositionOf(['*', 'bar'], { indices: Pathist.Indices.Ignore }), 5);

	// Wildcards match numbers
	t.is(p.lastPositionOf(['foo', '*'], { indices: Pathist.Indices.Ignore }), 0);
	t.is(p.lastPositionOf(['foo', -1], { indices: Pathist.Indices.Ignore }), 0);
});
