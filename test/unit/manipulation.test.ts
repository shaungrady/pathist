...
for (const { path, depth, expected, desc } of parentDepthCases) {
	test(`parent with ${desc}`, (t) => {
		...
		t.is(p.parentPath(depth).string, expected);
	});
}
